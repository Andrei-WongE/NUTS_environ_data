...
for(year in 2010:2020) {
  ...
  # Weight ERA5 by both latitude and population
  era5_weighted <- era5_europe * weights * pop_era5
  
  # Ensure extent matches exactly
  # ext(pop_era5) <- ext(era5_europe[[1]])
  
  # Calculate denominators (sum of weights) for each NUTS region
   weight_sums_nuts1 <- terra::extract(weights * pop_era5
                                       , nuts1
                                       , fun = 'sum'
                                       , na.rm = TRUE
                                       )
   
   weight_sums_nuts2 <- terra::extract(weights * pop_era5
                                       , nuts2
                                       , fun = 'sum'
                                       , na.rm = TRUE
                                       )
   
   # Extract weighted sums and divide by weight sums, exact=TRUE for more precise boundary calculations
   nuts1_means <- terra::extract(era5_weighted
                                , nuts1
                                , fun = 'sum'
                                , weights=TRUE
                                , exact = TRUE
   ...
   nuts2_means <- terra::extract(era5_weighted
                                , nuts2
                                , fun = 'sum'
                                , weights=TRUE
                                , exact = TRUE
   ...
   # Apply weights
   era5_data_a_nuts1_w[[as.character(year)]] <- data.frame(NUTS_ID = nuts1$NUTS_ID, 
                                                        value = nuts1_means)
   era5_data_a_nuts2_w[[as.character(year)]] <- data.frame(NUTS_ID = nuts2$NUTS_ID, 
                                                        value = nuts2_means)
}
...
saveRDS(era5_data_a_nuts1_w, here("Output","era5_data_a_nuts1_w.rds"))
saveRDS(era5_data_a_nuts2_w, here("Output", "era5_data_a_nuts2_w.rds"))
...
era5_historic <- terra::rast(here("Output","climate_data_era5-x0.25_historical_era5_climatology_1995-2014_combined.nc"))
crs(r) <- "EPSG:4326"

## Collection cmip6-xO.25_A: 2040-2059
cmip6_future <- terra::rast(here("Output","climate_data_cmip6-x0.25_ssp245_ssp585_climatology_2040-2059_combined.nc"))
crs(r) <- "EPSG:4326"

## Collection cmip6-xO.25 _B: 2040-2059
cmip6_historic <- terra::rast(here("Output","climate_data_cmip6-x0.25_ssp245_ssp585_climatology_2040-2059_combined.nc"))
crs(r) <- "EPSG:4326"

## popcount 2040-2059/2040 by scenario
...
### Ensure same extent
pop_raster_ext <- crop(pop_raster[["pop_2015"]], 
                             ext(pop_anomaly_ssp585),
                             snap = "near") #Snaps to the nearest pixel boundaries

### Then aggregate
pop_raster_resampled <- aggregate(pop_raster_ext, 
                                  fact = 30,  # 30 arc-seconds to 0.25 degrees 
                                  fun = "sum")

# Verify extent and resolution match
compareGeom(pop_fut_ssp585, pop_raster[["pop_2015"]])
compareGeom(pop_fut_ssp245, pop_raster[["pop_2015"]])
...
for(var in non_rain_vars) {
 anomaly <- cmip6_future[[paste0(var,"_ssp585")]] - cmip6_historic[[var]]
 future_ssp585[[var]] <- era5_historic[[var]] + anomaly
}
...
for(var in rain_vars) {
 ratio <- (cmip6_future[[paste0(var,"_ssp585")]] / cmip6_historic[[var]]) - 1
 future_ssp585[[var]] <- era5_historic[[var]] * (1 + ratio)
}
...
# Collection era5-xO.25_B + Collection cmip6-xO.25 _A
for(var in non_rain_vars) {
 anomaly <- cmip6_future[[paste0(var,"_ssp245")]] - cmip6_historic[[var]]
 future_ssp245[[var]] <- era5_historic[[var]] + anomaly
}
...
# Collection era5-xO.25_B *(1+ Deltha change)
for(var in rain_vars) {
 ratio <- (cmip6_future[[paste0(var,"_ssp245")]] / cmip6_historic[[var]]) - 1
 future_ssp245[[var]] <- era5_historic[[var]] * (1 + ratio)
}
```
