...
# Apply logitudinal weights to eras5 data
era5_weighted <- era5_europe * weights

# Create masked weight layers of eras5 to NUTS boundaries
era5_data_nuts1  <- mask(era5_weighted, nuts1_rast)
era5_data_nuts2  <- mask(era5_weighted, nuts2_rast)

# Process population data to match extent and resolution of era5
pop <- extend(pop_raster, ext(era5_europe))
...
for(year in 2010:2020) {
   ...
   pop_year <- pop[[paste0("pop_", year)]]
    
   pop_era5 <- aggregate(pop_year, fact = round(0.25/0.008333), fun = "sum")

   pop_nuts1[[year]] <- mask(pop_era5, nuts1_rast)
   pop_nuts2[[year]] <- mask(pop_era5, nuts2_rast)

}

# Apply yearly NUTS level population weights to eras5 data
era5_data_a_nuts1_w <-mapply(function(year) {
  
  weighted.mean(era5_data_nuts1, pop_nuts1[[year]])
}, 2010:2020)

era5_data_a_nuts2_w <- mapply(function(year) {
  
  weighted.mean(era5_data_nuts2, pop_nuts2[[year]])
}, 2010:2020)
...
compareGeom(era5_data_a_nuts1_w, era5_data_a_nuts2_w)

# Verify temporal dimension
print(paste("Time steps in original:", nlyr(era5_europe)))
print(paste("Time steps in NUTS1:", nlyr(era5_data_a_nuts1_w)))
print(paste("Time steps in NUTS2:", nlyr(era5_data_a_nuts2_w)))
...
print(paste("Number of NUTS1 regions with data:", 
           global(era5_data_a_nuts1_w, fun="notNA")[1,1]))
print(paste("Number of NUTS2 regions with data:", 
           global(era5_data_a_nuts2_w, fun="notNA")[1,1]))
```
...
aggregate_by_nuts <- function(data, nuts_rast, nuts_ids) {
  
   regional_means <- lapply(nuts_ids, function(id) {
       region_mask <- nuts_rast == id
       
       # Latitudinal weighting
       region_data <- mask(data, region_mask)
       region_weights <- mask(weights, region_mask)
       
       # Variable mean per region
       weighted.mean(region_data, w = region_weights, na.rm = TRUE)
   })
   names(regional_means) <- nuts_ids
   return(regional_means)
}

nuts1_regional <- aggregate_by_nuts(era5_data_b_nuts1_w, nuts1_rast, unique(nuts1$NUTS_ID), weights)
nuts2_regional <- aggregate_by_nuts(era5_data_b_nuts2_w, nuts2_rast, unique(nuts2$NUTS_ID), weights)
```
